import React from 'react'
import { Button } from './Button'

export const BottomNav: React.FC = () => {
  return (
    <div
      className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-white rounded-full px-8 py-2"
      style={{
        boxShadow:
          '0 1px 2px 0 rgba(5,26,36,0.08), 0 4px 16px 0 rgba(5,26,36,0.1), 0 24px 48px 0 rgba(5,26,36,0.08), inset 0 1px 0 0 rgba(255,255,255,0.8)',
      }}
    >
      <span
        className="text-2xl font-semibold text-[#051A24] leading-none"
        style={{ fontFamily: "'PP Mondwest', Georgia, serif" }}
      >
        V
      </span>
      <Button variant="primary">Start a chat</Button>
    </div>
  )
}
